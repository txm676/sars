...
cat_roxygen <- function(model, funName, fileName){
  ...
  cat1(paste0("#' @param ", "start ", "NULL or custom parameter start values",
  ...
  cat1(paste0("#'   or FALSE to prevent any grid start after a fail in initial", 
              " optimization", "\n"))
  cat1(paste0("#'   to run a grid search.", "\n"))
  cat1(paste0("#' @param ", "normaTest ", "The test used to test the normality", 
  ...
  cat1(paste0("#'   and the \\code{\\link{optim}} function. To avoid numerical", 
              " problems and speed up the convergence process,", "\n")) 
  cat1(paste0("#'   the starting values used to run the optimization algorithm", 
              " are carefully chosen, or custom values can be provided", "\n")) 
  cat1(paste0("#'   using the argument \\code{start}. The fitting process also", 
              " determines the observed shape of the model fit,", "\n")) 
  cat1(paste0("#'   and whether or not the observed fit is asymptotic (see", 
              " Triantis et al. 2012 for further details).", "\n\n"))
  cat1(paste0("#'   Model validation is undertaken by assessing the normality", 
              " (\\code{normaTest}) and homogeneity (\\code{homoTest})", "\n")) 
  ...
  cat1(paste0("#'   (see also \\code{\\link{fit_collection}} and", 
              " \\code{\\link{sar_multi}}).", "\n")) 
  ...
  cat1(paste0("#'     \\item{message} { Any message from the model fit", 
              " algorithm}", "\n"))
  cat1(paste0("#'     \\item{hessian} { A symmetric matrix giving an estimate", 
              " of the Hessian at the solution found}", "\n"))
  ...
  cat1(paste0("#' @references Triantis, K.A., Guilhaumon, F. & Whittaker, R.J.", 
              " (2012) The island species-area", "\n")) 
