...
While we provide three plotting options associated with sar_countryside, it should be noted that it is not straightforward to generate standard SAR model plots (i.e., with the fitted curve displayed alongside the observed data points) for countryside models as the same total habitat area (i.e., the total area of a given site) can be made up of different amounts of component habitats (e.g., 50% agriculture and 50% forest vs. 90% agriculture and 10% forest).

Finally, the fourth ('type' = 4) generates the “effective area” plots presented in @Merckx2019. The “effective area” for species group i in a site comprising j habitats is given by Ai = ∑j hijAj, where hij is the affinity of species group i to habitat j and is taken from the fitted sar_countryside model.
