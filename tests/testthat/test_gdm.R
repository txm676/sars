...
test_that("gdm functions return correct results", {
  ...
  g2 <- gdm(galap, model = "power", mod_sel = TRUE)
  pow <- g2[[3]]
  ...
  expect_error(gdm(aegean), "Not enough columns/variables to fit GDM")
  expect_error(gdm(galap, start_vals = c(5,4,3,6)), 
               "start_vals should be a dataframe with 1 row and four columns")
  expect_error(gdm(galap, model = "betap"), "provided model name not available")
  expect_error(gdm(galap, mod_sel = "d"), 
               "mod_sel argument should be TRUE or FALSE")
  #matches with BAT values
  BT <- BAT::gdm(galap$s, area = galap$a, time = galap$t)
  ...
  g3 <- gdm(galap, model = "loga", mod_sel = TRUE)
  loga <- g3[[3]]
  ...
  g4 <- gdm(galap, model = "linear", mod_sel = TRUE)
  linear <- g4[[3]]
  ...
  g5 <- gdm(galap, model = "all", mod_sel = FALSE)
  ...
  g6b <- gdm(galap, model = "lin_pow", mod_sel = FALSE)
