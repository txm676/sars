...
plot.coleman <- function(x, ps = 2.8, ls1 = 1.2, ls2 = 1.2, as1 = 14, as2 = 13,
                         c1 = "darkblue", c2 = "black", c3 = "darkred",
                         xl = "Relative area (log transformed)", yl = "Species richness"){
  ...
  ggplot2::ggplot(data = df) + ggplot2::geom_point(ggplot2::aes(x = log(ra),
                                                                y = os), size = ps, col = c1) +
    ggplot2::theme_bw() + ggplot2::geom_line(ggplot2::aes(x = log(ra), y = pv), col = c2, size = ls1) +
