...
#'   habitat_positions <- vector("list", length = length(habitat_names))
#' 
...
#' #'   island/site.
#' #' @param sp A vector of model names.
#' #' @param lurast The test used to test the normality of the residuals of each
...
#' #' @param habNam The test used to check for homogeneity of the residuals of
...
#' #' @param habVals The correlation test to be used when \code{homoTest !=
#' #'   "none"}. Can be any of "spearman" (the default), "pearson", or "kendall".
#' #' @param sq_width Should a grid search procedure be implemented to test
#' #'   multiple starting parameter values. Can be one of 'none', 'partial' or
#' #'   'exhaustive' The default is set to 'partial'.
#' #' @param cluster_size_vector If \code{grid_start = exhaustive}, the number of points sampled
...
#'   
#'   
#'   points <- st_as_sf(data, coords=c("long","lat"), crs=crs)
#' 
...
#' 
#'   res <- summarize_samples(list_flatten(samples), list_flatten(polygons),
#'                            lurast, habNam, habVals, species_groups,
